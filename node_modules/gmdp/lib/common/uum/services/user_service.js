...
exports.SaveListUser=function (user_data,cb) {
    try{
        // 按规则设置密码
        var password = user_data.login_account + config.project.password_suffix;
        user_data.login_password = utils.encryptDataByMD5(password);
        // 实例模型，调用保存方法
        model.$userListModel(user_data).save(function(error){
            if(error) {
                cb(utils.returnMsg(false, '1000', '新增用户时出现异常', null, error));
            }
            else {
